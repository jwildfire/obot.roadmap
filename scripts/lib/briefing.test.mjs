...
test('the theme is complete in both directions', async () => {
  const html = await render(EMPTY_DATA())
  // The three theme states moved into the shared document sheet on 2026-08-21 (#294,
  // under #289), so this page no longer states them itself — it links the sheet that
  // does. The assertion follows the guarantee rather than the bytes: what must remain
  // true is that a reader who has chosen a theme gets it, and the sheet is where that
  // is now decided and tested (obot.agent tools/style/test/census.test.mjs).
  assert.match(html, /<link rel="stylesheet" href="\.\.\/\.\.\/assets\/obot\.css">/,
    'the page takes its palette and all three theme states from the shared sheet')
  assert.doesNotMatch(html, /--paper:\s*#/, 'and does not restate a palette of its own')
  assert.match(html, /body \{[^}]*background: var\(--paper\)/s, 'a transparent body borrows the host ground')
})

test('the shared sheet it links is actually deployed beside it', async () => {
  // The link above is only a guarantee if the file is there. site/assets/obot.css is
  // vendored from obot.agent and copied into _site by the deploy; if it ever stops
  // being committed, every page that adopted it silently loses its palette.
  const vendored = new URL('../../site/assets/obot.css', import.meta.url)
  const css = await fs.readFile(vendored, 'utf8')
  assert.match(css, /obot shared stylesheet/, 'the vendored sheet is the shared one')
  assert.match(css, /VENDORED — do not edit this file/, 'and it carries the provenance header the deploy greps for')
  assert.match(css, /:root\[data-theme="dark"\]/, 'including the explicit-toggle block this page relies on')
})
