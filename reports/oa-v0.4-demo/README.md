...
Four sections, each pairing what an operator can now do with a real capture and the command that produced it:
...
- Every terminal capture is real output taken from this machine after the v0.4.0 release commit (`092e6fc`), not a mock-up. Terminal colour is reproduced in CSS; escape sequences appear as `^[` / `^G` exactly as `cat -v` prints them.
...
- Draft release: https://github.com/jwildfire/obot.agent/releases
