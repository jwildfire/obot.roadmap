...
read it aloud. It answers three questions in @jwildfire's terms: which goals are
...
renders it every night from GitHub alone: the open goal issues, their sub-issue trees,
the latest nightly comment on each goal, every issue carrying the `blocked` label, and
...
A note dictated in voice about a goal, a requirement or a blocked question is a comment
