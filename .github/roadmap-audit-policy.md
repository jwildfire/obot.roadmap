...
- Assignee `jwildfire` and milestone `backlog` on anything you file, plus the `requirement` label if it is a requirement. Add it to the obot Roadmap project (`gh project item-add 1 --owner jwildfire --url <url>`); if the token cannot see the project, say "board add deferred" in the report rather than failing. A board write is the one thing that cannot be attributed to the bot: the board is user-owned, and no GitHub App installation can reach a user-owned ProjectsV2 board — GitHub's REST reference says so on every user-owned project endpoint, and the only Projects permission it offers is organization-scoped. Board writes therefore go out under his name whatever you do; say so rather than letting it pass unremarked.
- **One requirement, one release** (@jwildfire, 2026-08-15). A requirement covers exactly one release. If a finding hands you a requirement whose delivered scope has shipped while other scope stays open, the fix is the split, in this order: note the deferral on the original (what and why), file a new requirement with its own milestone, **transfer** the deferred sub-issues (`removeSubIssue` then `addSubIssue` — never close-and-refile), then close the original with its release and set its board Status to Released. Never leave a requirement open because a later phase is planned; the later phase is a different requirement. Two cases need no new requirement: a defect found after release (an ordinary issue — re-home it to the goal), and scope that already has a requirement of its own and was merely nested (re-home it to the goal). Full statement in [README — One requirement, one release](../README.md#one-requirement-one-release).
...
- **Every write is attributed to `obotclaw[bot]`, never to @jwildfire.** The ambient `gh` token authenticates as him, and GitHub records the actor on the timeline, so a label or parent link written that way says he did it. For two days it said exactly that, on roughly a hundred issues he had not read ([obot.agent#197](https://github.com/jwildfire/obot.agent/issues/197)). In this workflow `GH_TOKEN` is already the app token, so a plain `gh` command is correct here. Running locally, put every write through `obot.agent/scripts/obot-gh`, which mints the token for you. The one exception is the board, below.
